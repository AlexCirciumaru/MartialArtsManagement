...
<div class="container myrow-container">
	<div class="panel panel-success">
		...
		<div class="panel-body">
			<c:choose>
				<c:when test="${empty degrees or empty degrees.content}">There are no Degrees</c:when>
				<c:otherwise>
					<form:form modelAttribute="searchDTO" method="get">
						...
						<div class="form-group">
							<label for="sortBy">Sort By:</label> <select name="sortBy"
								class="form-control">
								<option value="degreeRank">Degree Rank</option>
								<option value="degreeType">Degree Type</option>
								<option value="beltColor">Belt Color</option>
							</select>
						</div>
					</form:form>
					...
					<table class="table table-hover table-bordered">
						...
						<tbody>
							<c:forEach items="${degrees.content}" var="degree">
								<tr>
									<th><c:out value="${degree.id}" /></th>
									<th><c:out value="${degree.degreeRank}" /></th>
									<th><c:out value="${degree.degreeType}" /></th>
									<th><c:out value="${degree.beltColor}" /></th>
									<th><a
										href="<c:url value='/degree/update?id=${degree.id}'/>">Edit</a></th>
									<th><a
										href="<c:url value='/degree/delete?id=${degree.id}'/>">Delete</a></th>
								</tr>
							</c:forEach>
						</tbody>
					</table>
					
					${degrees.totalElements} total results
					...
					<ul class="pager">
						...
						<li class="">Page ${searchDTO.pageNumber} of ${degrees.totalPages}</li>

						<c:choose>
							<c:when test="${degrees.totalPages eq searchDTO.pageNumber}">
